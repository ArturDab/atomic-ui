            ...
            {TEAMS.map(team => (
              <button key={team.id} className={cn(
                ...
                <div className="flex items-center gap-1.5 mb-2">
                  {team.agents.map((a, i) => (
                    <div key={i} className={cn('w-6 h-6 rounded-full border flex items-center justify-center text-[10px] font-bold', team.agentColors[i])}>
                      {a}
                    </div>
                  ))}
                </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-1.5 mt-0.5">
              {AGENTS_CONFIG.filter(a => a.enabled).map(a => (
                <div key={a.id} className={cn('w-5 h-5 rounded-full border flex items-center justify-center text-[9px] font-bold', AGENT_COLOR[a.id])}>
                  {a.initial}
                </div>
              ))}
            ...
            {MESSAGES.map(msg => {
              ...
              return (
                <div key={msg.id} className="flex gap-3">
                  <div className={cn('w-7 h-7 rounded-full border flex items-center justify-center text-xs font-semibold shrink-0 mt-0.5', AGENT_COLOR[msg.agent!])}>
        ...
        <div className="w-64 border-l flex flex-col shrink-0 overflow-y-auto bg-background">
          ...
          <div className="p-4 space-y-5">
              ...
              <div className="space-y-2">
                {agents.map(a => (
                  <div key={a.id} className="flex items-center gap-3 p-2.5 border rounded-xl">
                    <div className={cn('w-7 h-7 rounded-full border flex items-center justify-center text-xs font-semibold shrink-0', AGENT_COLOR[a.id])}>
                      {a.initial}
                    </div>
                    <div className="flex-1 min-w-0">
            ...
            <div>
              ...
              <div className="border border-dashed rounded-xl flex flex-col items-center py-5 gap-2 text-muted-foreground">
