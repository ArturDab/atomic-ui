...
const STORAGE_KEY = 'lyra-ds-customizations'
const VERSIONS_KEY = 'lyra-ds-versions'
...
const COLOR_TOKENS = [
  // Tła
  { key: '--background',        label: 'Tło edytora (główne)',     group: 'Tła' },
  { key: '--card',              label: 'Tło kart',                 group: 'Tła' },
  { key: '--muted',             label: 'Tło poboczne',             group: 'Tła' },
  { key: '--sidebar',           label: 'Panel boczny (główny)',    group: 'Panele boczne' },
  { key: '--sidebar-light',     label: 'Panel nagłówków (jaśniej)',group: 'Panele boczne' },
  { key: '--sidebar-header',    label: 'Belki nagłówkowe paneli',  group: 'Panele boczne' },
  // Tekst
...
const FONT_OPTIONS = [
  { value: "'Geist', system-ui, sans-serif",          label: 'Geist',          type: 'sans' as const, preview: 'Aa Bb Cc 123' },
  { value: "'DM Sans', system-ui, sans-serif",        label: 'DM Sans',        type: 'sans' as const, preview: 'Aa Bb Cc 123' },
  { value: "'Inter', system-ui, sans-serif",          label: 'Inter',          type: 'sans' as const, preview: 'Aa Bb Cc 123' },
  { value: "system-ui, -apple-system, sans-serif",    label: 'System UI',      type: 'sans' as const, preview: 'Aa Bb Cc 123' },
  { value: "'Lora', Georgia, serif",                  label: 'Lora',           type: 'serif' as const, preview: 'Szybki brązowy lis przeskoczył' },
  { value: "'Merriweather', Georgia, serif",          label: 'Merriweather',   type: 'serif' as const, preview: 'Szybki brązowy lis przeskoczył' },
  { value: "'Source Serif 4', Georgia, serif",        label: 'Source Serif 4', type: 'serif' as const, preview: 'Szybki brązowy lis przeskoczył' },
  { value: "Georgia, serif",                          label: 'Georgia',        type: 'serif' as const, preview: 'Szybki brązowy lis przeskoczył' },
]
...
export default function DesignConfigPanel({ themeId, onVarsChange }: DesignConfigPanelProps) {
  ...
  const groups = COLOR_TOKENS.reduce((acc,t)=>{if(!acc[t.group])acc[t.group]=[];acc[t.group].push(t);return acc},{} as Record<string,typeof COLOR_TOKENS>)
        ...
        <div className="px-4 py-4 space-y-5">
          ...
          {activeTab==='colors' && Object.entries(groups).map(([groupName,tokens])=>(
            <div key={groupName}>
              ...
              {groupName==='Panele boczne'&&(
                <div className="mb-3 p-2.5 bg-muted/40 rounded-lg border text-[10px] text-muted-foreground leading-relaxed">
                  <strong className="text-foreground/70">Hierarchia jasności:</strong> Belki &gt; Panel główny &gt; Panel nagłówków &gt; Edytor
                </div>
              )}
                ...
                {FONT_OPTIONS.filter(f=>f.type==='serif').map(font=>(
                  <button key={font.value} onClick={()=>updateVar('--editor-font',font.value)}
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5 line-clamp-1"style={{fontFamily:font.value}}>{font.preview}</p>
                    </div>
                ...
                {FONT_OPTIONS.filter(f=>f.type==='sans').map(font=>(
                  <button key={font.value} onClick={()=>updateVar('--font-sans',font.value)}
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">Nawigacja · Przyciski · Panel AI</p>
                    </div>
