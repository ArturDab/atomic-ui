        ...
        <div className="flex-1 flex flex-col overflow-hidden min-w-0">
          ...
          <div className="h-9 border-b flex itely-center px-4 gap-2 bg-muted/20 shrink-0">
            ...
            <Button variant="ghost" size="icon" className="h-7 w-7" onClick={() => setShowAI(o => !o)}>
              <Sparkles className={cn('w-4 h-4', showAI ? 'text-foreground' : 'text-foreground/55')} />
            </Button>
          </div>
                ...
                <div className="grid grid-cols-3 gap-4">
                  {[['68%', 'Firm używa AI'], ['82%', 'Rekomendacje'], ['34pp', 'Wzrost YoY']].map(([val, label]) => (
                    <div key={label} className="text-center">
                      ...
                      <p className="text-xs text-foreground/65 mt-0.5">{label}</p>
                    </div>
                  ))}
                </div>
              </div>

              <p className="text-base text-foreground/55 mt-6">Kontynuuj pisanie...</p>
