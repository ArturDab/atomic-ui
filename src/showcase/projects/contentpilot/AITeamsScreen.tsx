      ...
      <div className="w-72 border-r flex flex-col shrink-0">
        <div className="p-4 border-b">
          <Button className="w-full gap-2">
            <Plus className="w-4 h-4" /> Utwórz zespół
          </Button>
        </div>
            ...
            <div className="space-y-2">
              {agents.map(agent => (
                <div key={agent.id} className="flex items-center gap-3 p-2.5 border rounded-lg">
                  ...
                  <Switch checked={agent.enabled} onCheckedChange={() => setAgents(prev => prev.map(a => a.id === agent.id ? { ...a, enabled: !a.enabled } : a))} />
                </div>
              ))}
            </div>
          </div>
